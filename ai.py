...
1. 聽住錄音，**根據聲線、語氣、節奏判斷有幾多位講者**
2. 用 `講者 A`、`講者 B`、`講者 C` ... 嚟標記（按出現順序）
3. **如果聽到 client 同 advisor / boss 同 employee 等明顯角色**，可以喺最後加 note 註明（例：「講者 A 似 audit partner」）
4. **如果分唔到（例如只有一個 speaker）**：用 `講者 A` 標記，唔好淨係寫文字無 speaker label
...
⚠️ 時間戳係 optional（識別到先加），但 `**講者 X**:` 標籤**必須有**。
...
6. **聽唔清**：用 `[聽唔清]` 標記
7. **同一講者連續講超過 30 秒** → 分成幾段，每段都用同一個 `**講者 X**:` 標籤
