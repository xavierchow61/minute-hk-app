...
## 👥 與會者
（聽錄音聲線判斷講者數量，列出每位 + 角色推斷。例：「**講者 A** — 似 audit partner」「**講者 B** — 似 client CFO」）

## 🎯 重點摘要
（1-2 段，~150 字）

## ✅ Action Items
（**負責人**用講者標籤，例：「**講者 A** 11/15 前提交修訂預算」；如果只有「講者 X」唔清楚實名，照寫）
...
## 👥 與會者（Speaker Diarization）
（聽錄音判斷有幾多位講者；每位用 `講者 A / B / C ...` 標籤，加角色推斷）
- **講者 A** — 角色推斷（例：似 audit partner / 主持人）；主要 contribution
- **講者 B** — 角色推斷；主要 contribution
- ...
...
## 👥 與會者
（聽錄音判斷有幾多位講者；每位用 `講者 A / B / C ...` 標籤 + 角色推斷）
- **講者 A** — 角色推斷
- **講者 B** — 角色推斷
- ...
...
【⚠️ 錄音質量檢查 - 首先判斷】
- 如果錄音**幾乎全靜音、只有噪音、或者完全聽唔到任何內容**
  → **直接返回**呢段（一字不漏，唔好加其他內容）：
  ```
  # ⚠️ 錄音質量問題
  AI 無法識別錄音內容。可能原因：
  - 麥克風冇收到聲音
...
【🛑 絕對禁止】
- **唔可以重複輸出同一個字**（如「喂喂喂喂」、「啊啊啊」、「ok ok ok」）
- 如果只聽到少量內容 → 老實 transcribe 你聽到嘅 + 標記 [內容不完整]，唔好 hallucinate
...
# ============================================================
TRANSCRIBE_ONLY_PROMPT = """請將呢段錄音逐字 transcribe 成文字稿，**必須做 speaker diarization (識別講者)**。
...
**講者 A** [00:00]: Hello,大家好，歡迎收聽今集嘅 podcast。今集我哋繼續傾偈。

**講者 B** [00:08]: Hello。

**講者 A** [00:09]: 咁呢，我哋上一集就講 AI 嘛，今集我想講下你過去一段時間喺美國 explore 唔同嘅創業。

**講者 B** [00:18]: 嗯。

**講者 A** [00:19]: 咁所以呢，我都幾了解 Silicon Valley 依家嗰個狀況。
```

注意嗰幾樣關鍵 element：

1. **講者 label 必須係完整 `**講者 A**` / `**講者 B**` / `**講者 C**` ...**
   - ❌ 唔好淨係寫 `A:` 或者 `B:`（短 form）
   - ❌ 唔好寫 `Speaker A` / `Person A`（英文）
   - ✅ 必須係**繁體中文「講者」+ 大階英文字母** 加埋 markdown 粗體
2. **每個 turn 必須係獨立一段，turn 之間用空行（雙換行 \\n\\n）分隔**
   - ❌ 唔好將 「**講者 A** ... **講者 B** ... **講者 A** ...」 寫晒喺同一段
   - ✅ 每次講者一轉就**換行 + 空一行 + 開新一段**
3. **時間戳 `[mm:ss]`** — 識別到先加（optional），但**講者 label 一定要**
4. **第一行**永遠係 `**👥 與會者**: 講者 A、講者 B、...（共 N 位）`，跟住一個空行先開始 dialog

# 講者識別規則

- 根據**聲線、語氣、節奏**判斷幾多位講者
- 按出現順序 assign `講者 A / B / C / ...`
- **單人錄音都要用 `講者 A`** label，唔好 skip
- 同一人短時間講多次都係**同一個 label**

# 轉錄要求

1. **繁體中文**輸出（簡體 → 繁體）
2. **保留原話**：唔好總結、唔好修飾、唔好刪減
3. **中英夾雜照保留**（例：「我哋個 audit 已經 finalize 咗」唔好譯）
4. **粵語口語照保留**（「嗰個」、「咁樣」、「點解」等）
5. **保留語氣詞**：「啊」「呢」「囉」、笑聲「(笑)」、停頓「(停頓)」等
6. **聽唔清**：用 `[聽唔清]` 標記

# 絕對禁止

- ❌ 唔好寫「會議重點」、「Action Items」等 summary 結構
- ❌ 唔好總結 / 修飾 / 加 markdown headings (例如 「## 主要議題」)
- ❌ 唔好加「由 AI 自動生成」之類嘅 footer
- ❌ 唔好用短 form 例如 `A:`、`B:`（永遠要 `**講者 A**`）
- ❌ 唔好將多個講者 turn 擠喺同一段（必須換行）
